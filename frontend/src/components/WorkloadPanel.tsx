...
import { Box, Card, CardContent, LinearProgress, Stack, TextField, Typography } from "@mui/material";
import dayjs from "dayjs";
import { apiClient } from "../api/client";
import type { AuditorWorkload } from "../types";

/** Plan de charge des auditeurs internes sur une période libre. */
export function WorkloadPanel() {
  ...
  useEffect(() => {
    apiClient
      .get<AuditorWorkload[]>("/api/planning/workload", { params: { start, end } })
  ...
  return (
    <Box>
      <Stack direction="row" spacing={2} sx={{ mb: 3 }}>
        <TextField
          type="date" label="Début période" size="small" InputLabelProps={{ shrink: true }}
          value={start} onChange={(e) => e.target.value && setStart(e.target.value)}
        />
        <TextField
          type="date" label="Fin période" size="small" InputLabelProps={{ shrink: true }}
          value={end} onChange={(e) => e.target.value && setEnd(e.target.value)}
        />
      </Stack>

      {workload.length === 0 && (
        <Typography color="text.secondary">Aucune charge assignée sur cette période.</Typography>
      )}
